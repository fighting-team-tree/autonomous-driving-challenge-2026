...
전체 데이터는 428GB인데 여유가 156GB입니다. [D-014](../../decisions.md#d-014)의
증분 처리로 갑니다 — **아카이브 하나씩 처리하고 원본을 지웁니다.**

```
아카이브 1개 받기 → 압축 해제 → make_91f.py → tar.gz·301f 삭제 → 91f만 보관
```

작업 공간은 아카이브 1개분이면 되고, 누적되는 건 91f 출력분뿐입니다.

**`livinglab`(9개)부터 시작하세요.** 세 지역 중 가장 작고, 파이프라인 검증에는
전체 데이터가 필요 없습니다. 첫 아카이브 하나로 다음을 재고 나머지를 계획하세요:

- tar.gz 크기 → 압축 해제 후 크기 → 91f 변환 후 크기
- `make_91f.py` 1개 처리 시간

이 세 값이 나오면 "몇 개까지 보유 가능한지"와 "전체 처리에 며칠 걸리는지"가
바로 계산됩니다.
...
| 측정 | 왜 필요한가 |
|---|---|
| **아카이브 1개의 tar.gz / 301f / 91f 크기** | 몇 개까지 보유 가능한지 계산의 근거 |
| `make_91f.py` 아카이브 1개 처리 시간 | 전체 전처리 소요일 추산 |
